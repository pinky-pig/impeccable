---
name: impeccable
description: "创建高辨识度、可投入生产的前端界面，强调真正的设计质量，避免泛化的 AI 审美。适用于用户要求构建 Web 组件、页面、设计作品、海报或应用，也适用于任何设计技能需要项目上下文时。可配合 'craft' 走完整的 shape-then-build 流程，配合 'teach' 建立设计上下文，或用 'extract' 抽取可复用组件与 tokens 进入设计系统。"
