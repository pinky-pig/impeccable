...
/**
 * Sync generated skill folders into a local config dir while preserving
 * any extra hand-maintained skills that don't exist in generated output.
 */
function syncGeneratedSkillsDir(src, dest) {
  fs.mkdirSync(dest, { recursive: true });

  const generatedEntries = fs.readdirSync(src, { withFileTypes: true });
  const generatedNames = new Set(generatedEntries.map((entry) => entry.name));
...
async function build() {
  ...
  for (const { provider, configDir } of syncConfigs) {
    ...
    if (fs.existsSync(skillsSrc)) {
      syncGeneratedSkillsDir(skillsSrc, skillsDest);
    }
  }
