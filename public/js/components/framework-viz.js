...
export class PeriodicTable {
	...
	showTooltip(el, cmd) {
		...
		let relParts = [];
			if (pairs.length > 0) relParts.push(`搭配 ${pairs.map(p => '/' + p).join(', ')}`);
			if (combinesWith.length > 0) relParts.push(`常与 ${combinesWith.map(p => '/' + p).join(', ')}`);
			if (leadsTo.length > 0) relParts.push(`后续可接 ${leadsTo.map(p => '/' + p).join(', ')}`);
